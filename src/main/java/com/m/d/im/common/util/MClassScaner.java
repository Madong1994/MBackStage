...
//        List<Class<?>> list = MClassScaner.getScanList(BaseHandleImpl.class, BaseHandleImpl.class);
    ...
    public static  List<Class> scanSubClass(Class clazzPackage , Class pclazz){
        List<Class>  scanList = null;
//        File scanfile = new File(clazzPackage.getResource("").getPath());
        String pa = clazzPackage.getPackage().getName();
        List<Class<?>> allClass = getClasses(pa);
        if(allClass != null){
            scanList = new ArrayList<Class>();
            for(Class classes : allClass) {
                // 判断是否是同一个接口
                if(pclazz.isAssignableFrom(classes)) {
                    // 本身不加入进去
                    if(!pclazz.equals(classes)) {
//                        pclazz.asSubclass(pclazz);
                        scanList.add(classes);
                    }
                }
            }
        }
